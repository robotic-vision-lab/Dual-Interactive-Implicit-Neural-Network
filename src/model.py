...
class Mark_3(nn.Module):
    def __init__(self, in_channels=1):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(in_channels, 16, 5, padding=2),
                                    nn.ReLU(),
        ...
        self.upsample = nn.Sequential(nn.Conv2d(128, 16, 3, padding=1),
                                        nn.ReLU(),
                                        nn.PixelShuffle(4))

        self.max_pool = nn.MaxPool2d(2)
        
        num_features = in_channels + 32 + 128 + 1

        self.mlp = nn.Sequential(nn.Conv2d(num_features, 128, 1),
                                nn.ReLU(),
                                nn.Conv2d(128, 1, 1),
                                nn.ReLU()) #if want RGB out change last output channel to 3
